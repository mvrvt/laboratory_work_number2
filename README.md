...
**Итого дополнительных баллов:** 12  
**Общая оценка (с учётом базовой части):** > 9 баллов (зачёт)

> **Примечание:** Не реализованы: `Builder`, `AdaptiveSequence`, `SegmentedList`, монады (`Option`/`Try`), `IGroup`/`IRing`, медиана, количество инверсий и некоторые другие задачи повышенной сложности – они не требовались для получения зачёта.
